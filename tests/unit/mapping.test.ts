...
describe('Unit: mapping module', () => {
  ...
  describe('mergeRevisionsWithVersions', () => {
    it('should merge revisions with version mappings', async () => {
      const { mergeRevisionsWithVersions } = await import(
        '../../src/lib/mapping.js'
      );
    ...
    it('should handle revisions without version mappings', async () => {
      ...
      const revisions = [{ revision: '999999', platform: 'Mac_Arm' }];

      const versionMappings: Array<{
        version: string;
    ...
    it('should deduplicate entries', async () => {
      ...
      const revisionCounts: Record<string, number> = merged.reduce(
  ...
  describe('compareVersions', () => {
    it('should compare version strings correctly', async () => {
      ...
      expect(compareVersions('93.0.4577.82', '92.0.4515.159')).toBeGreaterThan(
        0
      );
